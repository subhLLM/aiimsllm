    ...
    def get(self, user_id):
        ...
        with self.lock:
            try:
                ...
                # Test connection
                chat_history.add_user_message("__ping__")
                chat_history.messages.pop()  # clean test message
            except Exception as e:
                logger.warning(f"[MemoryStore] Redis unavailable, using in-memory history for user {user_id}: {e}")
...
try:
    ...
    logger.warning(f"Redis connection failed: {e}. Using fallback in-memory store.")
    user_memory_store = {}  # or use a simple in-memory dict fallback
...
def detect_query_complexity(query):
    ...
    # Rule 1: Coordination/conjunctions + enough length ⇒ complex
    if any(conj in query_lower for conj in [" and ", " or ", " but also ", " as well as "]) and token_count > 7: 
        return "complex"
    
    # Rule 2: Specific multi-intent / exploratory phrases ⇒ complex
    complex_phrases = [
    ...
    # Rule 3: Very short + direct info request ⇒ simple
    if token_count <= 5 and any(q_word in query_lower for q_word in [
        "where is room", "dr. email", "phone for cardiology"
    ]):
        return "simple"
    return "normal"
...
def normalize_query(query: str) -> str:
    ...
    # Room/OPD normalization (e.g., "opd3", "room 101a" → "opd-3", "room-101a")
    ...
    # Re-collapse repeated characters again
    ...
    # Punctuation cleanup (preserve '.', '-', '@')
    q = re.sub(r"[^\w\s\-\.@]", "", q)

    # Whitespace cleanup
    q = re.sub(r"\s+", " ", q).strip()

    # Normalize plural terms to singular
...
def canonicalize_entity_value(entity_value):
    ...
    # Step 1: Direct match via SYNONYM_MAP
    for canonical, aliases in SYNONYM_MAP.items():
        all_forms = [canonical.lower()] + [a.lower() for a in aliases]
        if value_l in all_forms:
            # Return the canonical key (e.g., "doctor" for "dr." or "dr")
            return canonical

    # Step 2: Fuzzy match fallback if no exact match
    ...
    # Step 3: Return original if no match found
    return entity_value
...
def generate_clarification_suggestions(entities, memory):
    ...
    # --- Doctor mentioned but department missing ---
    ...
    # --- Service mentioned without location ---
    ...
    # --- Department alone, unclear intent ---
    ...
    # --- Room mentioned without department ---
    ...
    # --- General fallback if nothing specific matched ---
...
def classify_query_characteristics(query: str) -> dict:
    ...
    long_phrases = ["explain in detail", "everything about", "detailed explanation"]
    medium_phrases = ["list", "summarize", "overview of", "comparison of", "tell me more", "a bit more detail", "summary of", "overview", "more info"]

    if any(phrase in query_l for phrase in long_phrases):
        response_length = "long"
    elif any(phrase in query_l for phrase in medium_phrases):
...
def detect_answer_style_and_tone(query: str) -> tuple:
    ...
    # --- Style Detection ---
    table_keywords = ["in a table", "tabular format"]
    bullet_keywords = ["bullet points", "list them", "pointwise", "in bullets", "quick list"]
    quick_answer_keywords = ["quick answer", "short answer"]

    if any(kw in query_l for kw in table_keywords):
        style = "table"
    elif any(kw in query_l for kw in bullet_keywords + quick_answer_keywords):
        style = "bullet_list"

    # --- Tone Detection ---
    friendly_keywords = ["casual", "friendly", "informal", "chill"]
    formal_keywords = ["formal", "official", "official statement", "professional tone", "be polite"]

    if any(kw in query_l for kw in friendly_keywords):
        tone = "friendly_and_casual"
    elif any(kw in query_l for kw in formal_keywords):
...
def rewrite_query_with_memory(query, memory: ConversationMemory):
    ...
    # Fallback: spaCy rewriting
    if rewritten_query == original_query and salient_topic_entity_value:
        rewritten_query_spacy = resolve_pronouns_with_spacy(original_query, salient_topic_entity_value)
        if rewritten_query_spacy and rewritten_query_spacy != original_query:
            rewritten_query = rewritten_query_spacy
            logger.info(f"[Coref Rewrite - spaCy] Rewrote '{original_query}' → '{rewritten_query}' using spaCy.")
            return rewritten_query
...
"""
            ...
            if llm_response and llm_response.lower() != original_query.lower():
                ...
                logger.info(f"[LLM Coref Rewrite] '{original_query}' → '{rewritten_query}'")
        except Exception as e:
...
def handle_small_talk(user_query: str, memory, user_id: str, detected_input_lang: str = "en", convo_intent: str = None) -> dict:
    ...
    # Check if user has had prior conversation
    has_prior_convo = False
    try:
        if memory and hasattr(memory, "chat_memory") and hasattr(memory.chat_memory, "messages"):
            has_prior_convo = len(memory.chat_memory.messages) > 1
    except Exception as e:
        logger.warning(f"[Small Talk] Could not check prior memory: {e}")
...
Assistant:"""
    ...
    try:
        llm = ChatGroq(api_key=GROQ_API_KEY, model="gemma2-9b-it", temperature=0.85)
    ...
    # Prevent repetition: Check if same as last assistant message
    try:
        if memory and hasattr(memory, "chat_memory") and hasattr(memory.chat_memory, "messages"):
            messages = memory.chat_memory.messages
            last_assistant_msgs = [m for m in reversed(messages) if m.type == "ai"]
            if last_assistant_msgs:
                last_reply = last_assistant_msgs[0].content.strip()
                if response.strip() == last_reply:
                    response += " 😊 Anything else I can assist you with?"
    except Exception as e:
        logger.warning(f"[Small Talk] Failed to check for repeated assistant message: {e}")

    # Save turn to LangChain memory (correct API)
    try:
        if hasattr(memory, "chat_memory"):
            memory.chat_memory.add_user_message(query_cleaned)
            memory.chat_memory.add_ai_message(response)
        user_memory_store.save(user_id, memory)
    except Exception as e:
        logger.error(f"[Small Talk] Failed to save memory: {e}")

    # Optional: Translate response to user's language
...
def chat(user_query: str, user_id: str):
    ...
    # Detect language *immediately* so it’s available for small talk too
...
if __name__ == "__main__":
    ...
    uvicorn.run("main22spacysarvam:app", host="0.0.0.0", port=5001, reload=True)