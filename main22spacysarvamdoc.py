...
class NLUProcessor:
    ...
    def extract_entities(self, query: str) -> dict:
        """
        Extracts entities using a cascade of NER models: English, Multilingual, Indic.
        Returns merged entity dictionary.
        """
        entities = {}

        try:
            if self.ner_pipeline_primary:
                ner_results = self.ner_pipeline_primary(query)
                for ent in ner_results:
                    ent_type = ent.get("entity_group")
                    ent_text = ent.get("word").strip()
                    if ent_type and ent_text:
                        entities.setdefault(ent_type, []).append(ent_text)

            # Fallback: Multilingual NER
            if not entities and self.ner_pipeline_fallback:
                fallback_results = self.ner_pipeline_fallback(query)
                for ent in fallback_results:
                    ent_type = ent.get("entity_group")
                    ent_text = ent.get("word").strip()
                    if ent_type and ent_text:
                        entities.setdefault(ent_type, []).append(ent_text)

            # Tertiary fallback: IndicNER (for Indian languages)
            if not entities and self.ner_pipeline_indic:
                indic_results = self.ner_pipeline_indic(query)
                for ent in indic_results:
                    ent_type = ent.get("entity_group")
                    ent_text = ent.get("word").strip()
                    if ent_type and ent_text:
                        entities.setdefault(ent_type, []).append(ent_text)
...
HOSPITAL_MODEL_JSON_PATH = "aa.json"
QA_PAIRS_JSON_PATH = "a.json"
...
class HospitalDataLoader:
    ...
    def load_json_secure(self, filename):
        """Securely load JSON file with error handling"""
    ...
    def _extract_known_entities(self):
        """Extract all known entities from both hospital and QA data"""
        entities = {
        ...
        # Extract from hospital data (aa.json structure)
        if self.hospital_data:
            ...
            campus = self.hospital_data.get("campus", {})
            
            # Hospital/Campus information
            if campus.get("name"):
                hospitals.add(campus["name"])
                venues.add(campus["name"])
            
            # Process departments
            for dept in ensure_list(campus.get("departments", [])):
                if dept.get("name"):
                    departments.add(dept["name"])
                
                # Process faculty/doctors in each department
                for faculty in ensure_list(dept.get("faculty", [])):
                    if faculty.get("name"):
                        doctors.add(faculty["name"])
                    
                    # Services from faculty
                    for service in ensure_list(faculty.get("servicesOffered", [])):
                        services.add(service)
                    
                    # Room information
                    room_info = faculty.get("room", {})
                    if room_info.get("number"):
                        rooms.add(str(room_info["number"]))
                    if room_info.get("venue"):
                        venues.add(room_info["venue"])
                    if room_info.get("building"):
                        buildings.add(room_info["building"])
                    
                    # Accessibility access points
                    access_points = room_info.get("accessibility", {}).get("nearestAccessPoints", {})
                    
                    for lift in ensure_list(access_points.get("lifts", [])):
                        if lift.get("name"):
                            lifts.add(lift["name"])
                    
                    for stair in ensure_list(access_points.get("stairs", [])):
                        if stair.get("name"):
                            stairs.add(stair["name"])
                    
                    for exit in ensure_list(access_points.get("emergencyExits", [])):
                        if exit.get("name"):
                            exits.add(exit["name"])
                    
                    for entrance in ensure_list(access_points.get("entrances", [])):
                        if entrance.get("name"):
                            entrances.add(entrance["name"])
                    
                    for ramp in ensure_list(access_points.get("ramps", [])):
                        if ramp.get("name"):
                            ramps.add(ramp["name"])
        ...
        # Extract from QA data (a.json structure)
        if self.qa_data:
            topics = set()
            sources = set()
            
            for qa_item in ensure_list(self.qa_data):
    ...
    def get_all_metadata_tags(self):
        """Get all metadata tags from both data sources"""
        tags = set()
        ...
        if self.hospital_data:
            ...
            # Campus area type as tag
            if campus.get("areaType"):
                tags.add(campus["areaType"].lower().replace(" ", "_"))
            
            # Department tags
            for dept in ensure_list(campus.get("departments", [])):
                if dept.get("name"):
                    tags.add(dept["name"].lower().replace(" ", "_"))
                
                # Faculty/Doctor tags
                for faculty in ensure_list(dept.get("faculty", [])):
                    if faculty.get("designation"):
                        tags.add(faculty["designation"].lower().replace(" ", "_"))
                    
                    if faculty.get("specialization"):
                        tags.add(faculty["specialization"].lower().replace(" ", "_"))
                    
                    # Room type tags
                    room_info = faculty.get("room", {})
                    if room_info.get("type"):
                        tags.add(room_info["type"].lower().replace(" ", "_"))
                    
                    # Service tags
                    for service in ensure_list(faculty.get("servicesOffered", [])):
                        tags.add(service.lower().replace(" ", "_"))
                    
                    # Accessibility tags
                    if room_info.get("accessibility", {}).get("isWheelchairAccessible"):
                        tags.add("wheelchair_accessible")
                    
                    # Faculty tags from existing tags field
                    for tag in ensure_list(faculty.get("tags", [])):
    ...
    def get_metadata_tag_counts(self):
        ...
        if self.hospital_data:
            ...
            # Count campus area type
            if campus.get("areaType"):
                counter[campus["areaType"].lower().replace(" ", "_")] += 1
            
            # Count departments
            for dept in ensure_list(campus.get("departments", [])):
                if dept.get("name"):
                    counter[dept["name"].lower().replace(" ", "_")] += 1
                
                # Count faculty designations and specializations
                for faculty in ensure_list(dept.get("faculty", [])):
                    if faculty.get("designation"):
                        counter[faculty["designation"].lower().replace(" ", "_")] += 1
                    
                    if faculty.get("specialization"):
                        counter[faculty["specialization"].lower().replace(" ", "_")] += 1
                    
                    # Count services
                    for service in ensure_list(faculty.get("servicesOffered", [])):
                        counter[service.lower().replace(" ", "_")] += 1
...
def _prepare_hospital_documents():
    """Prepare documents from hospital data (aa.json structure)"""
    documents = []
    
    if not data_loader.hospital_data:
        return documents
    
    campus = data_loader.hospital_data.get("campus", {})
    
    # Process each department and its faculty
    for dept_index, dept in enumerate(ensure_list(campus.get("departments", []))):
        dept_name = dept.get("name", f"Department_{dept_index}")
        
        # Process each faculty member in the department
        for faculty_index, faculty in enumerate(ensure_list(dept.get("faculty", []))):
            content_parts = []
            metadata_payload = {
                "source_doc_id": faculty.get("id", f"faculty_{dept_index}_{faculty_index}"),
                "document_type": "hospital_data",
                "type": "faculty_room"
            }
            
            # Campus and Department Context
            campus_name = campus.get("name", "Unknown Campus")
            campus_location = campus.get("location", "Unknown Location")
            content_parts.append(f"Campus: {campus_name}, Location: {campus_location}")
            content_parts.append(f"Department: {dept_name}")
            
            metadata_payload.update({
                "campus_name": campus_name,
                "campus_location": campus_location,
                "department_name": dept_name,
                "department_website": dept.get("website")
            })
            
            # Faculty Information
            faculty_name = faculty.get("name", "Unknown Faculty")
            designation = faculty.get("designation", "Unknown")
            specialization = faculty.get("specialization", "Unknown")
            about = faculty.get("about", "No description available")
            
            content_parts.append(f"Faculty: {faculty_name} ({designation})")
            content_parts.append(f"Specialization: {specialization}")
            ...
            if contact:
                ...
                if contact_info:
                    content_parts.append(f"Contact: {', '.join(contact_info)}")
            ...
            # Room Information
            room = faculty.get("room", {})
            if room:
                room_number = room.get("number", "Unknown")
                room_type = room.get("type", "Unknown")
                venue = room.get("venue", "Unknown")
                floor = room.get("floor", "Unknown")
                building = room.get("building", "Unknown")
                
                content_parts.append(f"Room: {room_number} ({room_type})")
                content_parts.append(f"Location: {venue}, Floor {floor}, Building {building}")
                ...
                if features:
                    ...
                    area = features.get("areaSqFt", "Unknown")
                    flooring = features.get("flooring", "Unknown")
                    
                    content_parts.append(f"Room Features: Capacity {capacity}, Area {area} sq ft, Flooring {flooring}")
                    
                    # Door information
                    door = features.get("door", {})
                    if door:
                        door_type = door.get("type", "Unknown")
                        motion = door.get("motion", "Unknown")
                        content_parts.append(f"Door: {door_type}, Motion {motion}")
                    
                    # Utilities
                    utilities = features.get("utilities", {})
                    if utilities:
                        util_list = []
                        for key, value in utilities.items():
                            util_list.append(f"{key}: {value}")
                        content_parts.append(f"Utilities: {', '.join(util_list)}")
                
                # Coordinates
                coordinates = room.get("coordinates", {})
                if coordinates:
                    geo = coordinates.get("geo", {})
                    cartesian = coordinates
                    coord_info = []
                    
                    if geo.get("latitude") and geo.get("longitude"):
                        coord_info.append(f"GPS: {geo['latitude']}, {geo['longitude']}")
                    if coordinates.get("x") and coordinates.get("y"):
                        coord_info.append(f"Cartesian: X={coordinates['x']}, Y={coordinates['y']}")
                    
                    if coord_info:
                        content_parts.append(f"Coordinates: {', '.join(coord_info)}")
                
                # Accessibility
                accessibility = room.get("accessibility", {})
                if accessibility:
                    wheelchair = accessibility.get("isWheelchairAccessible", False)
                    content_parts.append(f"Wheelchair Accessible: {wheelchair}")
                    
                    features_list = ensure_list(accessibility.get("features", []))
                    if features_list:
                        content_parts.append(f"Accessibility Features: {', '.join(features_list)}")
                    
                    # Nearest Access Points
                    access_points = accessibility.get("nearestAccessPoints", {})
                    ap_info = []
                    
                    for ap_type, ap_list in access_points.items():
                        for ap in ensure_list(ap_list):
                            name = ap.get("name", f"Unnamed {ap_type[:-1]}")
                            distance = ap.get("distanceMeters", "Unknown")
                            ap_info.append(f"{name} ({distance}m)")
                    
                    if ap_info:
                        content_parts.append(f"Nearest Access Points: {', '.join(ap_info)}")
                    
                    # Amenities
                    amenities = ensure_list(accessibility.get("amenities", []))
                    if amenities:
                        content_parts.append(f"Amenities: {', '.join(amenities)}")
                        metadata_payload["amenities"] = amenities[:5]
            
            # Availability
            availability = faculty.get("availability", {})
            if availability:
                operating_hours = availability.get("operatingHours", {})
                if operating_hours:
                    hours_str = format_operating_hours(operating_hours)
                    content_parts.append(f"Operating Hours: {hours_str}")
                
                note = availability.get("availabilityNote", "")
                if note:
                    content_parts.append(f"Availability Note: {note}")
            
            # Services Offered
            services = ensure_list(faculty.get("servicesOffered", []))
            if services:
                content_parts.append(f"Services Offered: {', '.join(services)}")
                metadata_payload["services"] = services[:10]
            
            # Booking Information
            booking = faculty.get("booking", {})
            if booking and booking.get("enabled"):
                booking_info = []
                if booking.get("method"):
                    booking_info.append(f"Method: {booking['method']}")
                if booking.get("url"):
                    booking_info.append(f"URL: {booking['url']}")
                if booking.get("approvalRequired"):
                    booking_info.append("Approval Required")
                if booking.get("note"):
                    booking_info.append(f"Note: {booking['note']}")
                
                if booking_info:
                    content_parts.append(f"Booking: {', '.join(booking_info)}")
            
            # Tags
            tags = ensure_list(faculty.get("tags", []))
            if tags:
                content_parts.append(f"Tags: {', '.join(tags)}")
                metadata_payload["tags"] = tags[:5]
            
            # Create final document
            page_content = "\n".join(filter(None, content_parts))
            metadata_payload["summary"] = f"{faculty_name} - {designation} in {dept_name}"
            metadata_payload["priority"] = 1
            
            documents.append(Document(page_content=page_content, metadata=metadata_payload))
    
    # Add helpdesk information as separate documents
    helpdesk = campus.get("helpdesk", {})
    if helpdesk:
        for service_type, contacts in helpdesk.items():
            content_parts = []
            metadata_payload = {
                "source_doc_id": f"helpdesk_{service_type}",
                "document_type": "hospital_data",
                "type": "helpdesk"
            }
            
            content_parts.append(f"Campus: {campus.get('name', 'Unknown')}")
            content_parts.append(f"Helpdesk Service: {service_type}")
            
            for contact_type, phone in contacts.items():
                content_parts.append(f"{contact_type}: {phone}")
            ...
            page_content = "\n".join(filter(None, content_parts))
            metadata_payload["summary"] = f"Helpdesk - {service_type}"
...
def _prepare_qa_documents():
    ...
    if not data_loader.qa_data:
        return documents
    
    for qa_index, qa_item in enumerate(ensure_list(data_loader.qa_data)):
        content_parts = []
        metadata_payload = {
        ...
        elif any(term in content_lower for term in ['doctor', 'physician', 'specialist', 'warden']):
        ...
        elif question.lower().startswith(('who is', 'who are')):
            metadata_payload["priority"] = 1  # High priority for person questions
        else:
...
class EnhancedNLUProcessor(NLUProcessor):
    def get_intent_labels(self):
        return super().get_intent_labels() + [
        "location",              # where is doctor/room/dept
        "contact_info",          # phone/email lookup
        "operating_hours",       # timings/schedule
        "booking_info",          # appointments
        "doctor_availability",   # when is doctor available
        "emergency_info",
        "accessibility_info",
        "general_information",
        "explanation",
        ...
        "service_info",
        "department_info",
        "facility_inquiry",           # (can merge with service_info)
        "medical_staff_inquiry",      # e.g., nurse info
        "listing_all",                # e.g., "list all departments"
        "listing_specific",           # e.g., "list all doctors in cardiology"
    ...
    def override_intent_if_needed(self, query, base_intent):
        ...
        if any(term in query_l for term in ['where', 'find', 'locate']) and any(term in query_l for term in ['room', 'ward', 'floor', 'building']):
            return "location"
...
try:
    fast_reranker = CrossEncoder('BAAI/bge-reranker-base', max_length=512, device=device) # Lightweight,Fast, multilingual, good for first pass
    strong_reranker = CrossEncoder('mixedbread-ai/mxbai-rerank-base-v1', max_length=512, device=device) # Heavier, high accuracy, mxbai-rerank-large-v1
    ...
    try:
        fast_reranker = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-6-v2', max_length=512, device=device)
        strong_reranker = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-12-v2', max_length=512, device=device)
...
CANONICAL_TERMS = [
    ...
    "appointment booking", "operating hours", "contact information",  # …
]
...
def expand_query_with_synonyms(query: str, max_variants: int = 6):
    ...
    # 1. Dictionary expansion (unchanged)
    for keyword, synonyms in SYNONYM_MAP.items():
        all_forms = [keyword] + synonyms
        for v in list(variants):
            for form in all_forms:
                if re.search(rf'\b{re.escape(form)}\b', v):
                    for repl in all_forms:
                        if repl != form:
                            variants.add(re.sub(rf'\b{re.escape(form)}\b', repl, v))
    ...
    if len(final) > 1 and query_lower not in final:
        logger.info(f"[SemanticSyn] '{query}' → {final}")
    return final
...
def ground_entity_to_docs(entity_value: str, docs, threshold=0.7):
    ...
    best_doc = None
    matched_field = None
    all_scores_log = []  # For traceability
    ...
    if best_score < threshold:
        ...
        logger.info(f"[Grounding Candidates for '{entity_value}']:\n" +
            "\n".join(f"DocID={item['doc_id']} | Field={item['field']} | Score={item['score']}" for item in sorted(all_scores_log, key=lambda x: -x["score"])[:5])
        )
...
def chat(user_query: str, user_id: str):
    ...
    # === Intent Detection ===
    task_type = detect_task_type(processed_query)
