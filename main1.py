...
class ConversationMemory:
    ...
    def get_last_entity_by_priority(self, type_priority=None):
        if type_priority is None:
            # Updated priority for hospital context
...
FAISS_INDEX_PATH = "rag_hospital_data_index"
...
def _prepare_hospital_documents():
    """Prepare documents from hospital data"""
    documents = []
    ...
    for item_index, item_data in enumerate(data_loader.hospital_data):
        ...
        room_details = item_data.get("roomDetails", {})
        source_id = f"{item_data.get('hospitalId', 'unknown_hospital')}_{room_details.get('locationId', f'item_{item_index}')}"
        ...
        # Hospital Information
        hospital_name = item_data.get("hospitalName", "N/A")
        hospital_type = item_data.get("hospitalType", "N/A")
        content_parts.append(f"Hospital: {hospital_name} ({hospital_type}).")

        # Location Context
        loc_ctx = item_data.get("locationContext", {})
        content_parts.append(
            f"Location Context: Venue '{loc_ctx.get('venueName', 'N/A')}', "
            f"Building '{loc_ctx.get('buildingName', 'N/A')}', "
            f"Zone '{loc_ctx.get('zone', 'N/A')}', "
            f"Area Type '{loc_ctx.get('areaType', 'N/A')}', "
            f"Indoor: {loc_ctx.get('indoor', 'N/A')}."
        )
        metadata_payload.update({
            "venue_name": loc_ctx.get("venueName"),
            "building_name": loc_ctx.get("buildingName"),
            "zone": loc_ctx.get("zone"),
            "area_type": loc_ctx.get("areaType")
        })

        # Room Details
        if room_details:
            room_name = room_details.get("roomName", "N/A")
            room_number = room_details.get("roomNumber", "N/A")
            floor = room_details.get("floor", "N/A")
            room_type = room_details.get("roomType", "N/A")
            room_subtype = room_details.get("roomSubType", "N/A")
            room_function = room_details.get("roomFunction", "General Purpose")
            
            content_parts.append(
                f"Room Details: Name '{room_name}', Number '{room_number}', "
                f"Floor {floor}, Type '{room_type}', SubType '{room_subtype}'. "
                f"Function: {room_function}."
            )
            
            metadata_payload.update({
                "room_location_id": room_details.get("locationId"),
                "room_name": room_name,
            ...
            geometry = room_details.get("geometry", {})
            geo = geometry.get("geo", {})
            cartesian = geometry.get("cartesian", {})
            ...
            if geo.get('latitude') and geo.get('longitude'):
                metadata_payload["geo_coordinates"] = f"{geo['latitude']},{geo['longitude']}"
            metadata_payload["coordinates_door_cartesian"] = door_coords

            # Door Information
            door_info = room_details.get("door", {})
            content_parts.append(
                f"Door: Type '{door_info.get('type', 'N/A')}', "
                f"Mechanism '{door_info.get('mechanism', 'N/A')}', "
                f"Motion '{door_info.get('motion', 'N/A')}'. "
                f"SmartLock: {door_info.get('smartLock', False)}."
            )

            # Room Utilities
            utilities = room_details.get("utilities", {})
        ...
        contact_str = format_contact_info(contact_info)
        content_parts.append(f"Hospital Contact: {contact_str}.")
        
        if contact_info:
            metadata_payload.update({
                "hospital_phone": contact_info.get("phoneNumbers"),
                "hospital_email": contact_info.get("email"),
                "hospital_website": contact_info.get("website")
            })
            
            address = contact_info.get("address", {})
            if address.get("city"):
                metadata_payload["hospital_address_city"] = address["city"]

        # Services Offered
        services_offered = ensure_list(item_data.get("servicesOffered", []))
        services_list = []
        ...
        for service in services_offered:
            ...
            specialties = ensure_list(service.get("specialties", []))
            specialties_str = ", ".join(specialties) if specialties else "N/A"
            
            content_parts.append(
                f"Service: {service_name} (ID: {service.get('serviceId', 'N/A')}). "
                f"Description: {service.get('description', 'N/A')}. "
                f"Department: {service.get('department', 'N/A')}. "
                f"Specialties: {specialties_str}."
            )
        
        if services_list:
            metadata_payload["services_offered"] = sorted(list(set(services_list)))

        # Departments and Doctors
        departments = ensure_list(item_data.get("departments", []))
        department_names = []
        doctor_names = []
        related_services = []
        ...
        for dept in departments:
            ...
            content_parts.append(
                f"Department: {dept_name} (ID: {dept.get('departmentId', 'N/A')}). "
                f"Floor: {dept.get('floor', 'N/A')}. "
                f"Contact: {dept.get('contactNumber', 'N/A')}."
            )
            
            # Department services
            dept_services = ensure_list(dept.get("relatedServices", []))
            related_services.extend(dept_services)
            if dept_services:
                content_parts.append(f"Department Services: {', '.join(dept_services)}.")
            
            # Department doctors
            for doctor in ensure_list(dept.get("doctors", [])):
                doc_name = doctor.get("name", "N/A")
                doctor_names.append(doc_name)
                
                availability = format_doctor_availability(doctor.get("availability", {}))
                
                content_parts.append(
                    f"Doctor: {doc_name} (ID: {doctor.get('doctorId', 'N/A')}). "
                    f"Specialization: {doctor.get('specialization', 'N/A')}, "
                    f"Designation: {doctor.get('designation', 'N/A')}. "
                    f"Email: {doctor.get('email', 'N/A')}, "
                    f"Phone: {doctor.get('phone', 'N/A')}. "
                    f"Availability: {availability}. "
                    f"Profile: {doctor.get('profile', 'N/A')}. "
                    f"About: {doctor.get('about', 'N/A')[:100]}..."
                )

        if department_names:
            metadata_payload["departments"] = sorted(list(set(department_names)))
        if doctor_names:
            metadata_payload["doctors"] = sorted(list(set(doctor_names)))
        if related_services:
            metadata_payload["department_services"] = sorted(list(set(related_services)))

        # Accessibility
        accessibility = item_data.get("accessibility", {})
        wheelchair_accessible = accessibility.get("isWheelchairAccessible", False)
        content_parts.append(f"Wheelchair Accessible: {wheelchair_accessible}.")
        metadata_payload["wheelchair_accessible"] = wheelchair_accessible
        
        acc_features = ensure_list(accessibility.get("features", []))
        acc_features_str = ", ".join(acc_features) if acc_features else "N/A"
        content_parts.append(f"Accessibility Features: {acc_features_str}.")
        metadata_payload["accessibility_features"] = acc_features
        ...
        for ap_type, ap_list in access_points.items():
            for ap_item in ensure_list(ap_list):
                ap_name = ap_item.get("name", f"Unnamed {ap_type[:-1] if ap_type.endswith('s') else ap_type}")
                distance = ap_item.get("distanceMeters", "N/A")
                ap_parts.append(f"{ap_name} ({distance}m)")
        ...
        metadata_payload["amenities"] = amenities

        # Emergency Services
        emergency_services = item_data.get("emergencyServices", False)
        content_parts.append(f"Emergency Services Available: {emergency_services}.")
        metadata_payload["emergency_services"] = emergency_services

        # Create synthetic tags for metadata
        tags = []
        if hospital_type:
            tags.append(hospital_type.lower().replace(" ", "_"))
        if room_details.get("roomType"):
            tags.append(room_details["roomType"].lower().replace(" ", "_"))
        if emergency_services:
            tags.append("emergency_services")
        if wheelchair_accessible:
            tags.append("wheelchair_accessible")
        
        metadata_payload["tags"] = tags[:5]  # Limit to 5 tags
        
        # Create summary
        summary_parts = [
            f"{hospital_name} {hospital_type}",
            f"Room: {room_details.get('roomName', 'N/A')}" if room_details else None,
            f"Departments: {len(departments)}" if departments else None,
            f"Services: {len(services_offered)}" if services_offered else None
        ]
        summary = " | ".join(filter(None, summary_parts))
        metadata_payload["summary"] = summary
        metadata_payload["priority"] = 1  # High priority for hospital data
...
class EnhancedNLUProcessor(NLUProcessor):
    ...
    def classify_intent(self, query):
        ...
        if any(term in query_lower for term in ['find', 'locate', 'where is']) and any(term in query_lower for term in ['room', 'ward']):
            return "find_room"
        elif any(term in query_lower for term in ['find', 'locate', 'where is']) and any(term in query_lower for term in ['department', 'unit']):
            return "find_department"
        elif any(term in query_lower for term in ['find', 'locate', 'who is']) and any(term in query_lower for term in ['doctor', 'physician', 'specialist']):
            return "find_doctor"
        elif any(term in query_lower for term in ['find', 'what', 'available']) and any(term in query_lower for term in ['service', 'treatment', 'procedure']):
            return "find_service"
        elif any(term in query_lower for term in ['contact', 'phone', 'email', 'call']):
            return "get_contact_info"
        elif any(term in query_lower for term in ['location', 'address', 'where is', 'directions']):
            return "get_location"
        elif any(term in query_lower for term in ['appointment', 'book', 'schedule', 'reserve']):
            return "get_schedule"
...
def get_embedding_model_for_query(query):
    ...
    if any(word in query_lower for word in ["explain", "describe", "details about", "how does", "procedure", "treatment", "everything about", "provide"]):
...
def detect_query_complexity(query):
    query_lower = query.lower()
    if any(conj in query_lower for conj in [" and ", " or ", " but also ", " as well as "]) and len(query.split()) > 7: 
        return "complex"
    if any(word in query_lower for word in ["list all services", "all doctors in department", "explain treatment options", "compare procedures"]): 
        return "complex"
    if len(query.split()) <= 5 and any(q_word in query_lower for q_word in ["where is room", "dr. email", "phone for cardiology"]): 
        return "simple"
    return "normal"
...
Assistant:"""
    ...
    try:
        ...
        response = random.choice([
            "Hey there! 👋 What can I help you with at Aimms Jammu?",
...
def chat(user_query: str):
    ...
    conv_memory = ConversationMemory() # Fresh instance for current request logic
    try:
