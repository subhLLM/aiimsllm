...
app.config["SESSION_FILE_DIR"] = "./flask_session_jammu/" # Changed session dir
...
def extract_doctor_name(text: str) -> str:
    """
    Extracts a doctor's name like 'Dr Aymen Masood' or 'Dr. Shruti Sharma' from query.
    """
    try:
...
def format_doctor_availability(avail):
    if not avail:
        return "N/A"
    return f"{', '.join(avail.get('days', []))} ({avail.get('time', 'N/A')})"
...
Assistant:"""
    ...
    try:
        llm = ChatOllama(model="mistral:7b-instruct", base_url="http://localhost:11434", temperature=0.85)
...
"""
    ...
    if response_length_hint == "long" or task_type in ["explanation", "comparison", "listing_all"] or "complex" in detect_query_complexity(processed_query):
        ollama_model_name, temperature_val = "llama3:8b", 0.4  # Slightly lower temp for long factual
    elif task_type in ["contact_info", "location", "doctor_availability"] and response_length_hint == "short":
        ollama_model_name, temperature_val = "llama3:8b", 0.15  # Precise for short facts
    else:
        ollama_model_name, temperature_val = "llama3:8b", 0.25  # Default to capable and fast

    logger.info(f"Using Ollama model (hospital): {ollama_model_name} with temperature: {temperature_val}")

    # Use Ollama instead of Groq
    from langchain_community.chat_models import ChatOllama
    llm = ChatOllama(model=ollama_model_name, base_url="http://localhost:11434", temperature=temperature_val)
...
def home():
    ...
    return render_template("indextag.html") 
...
@app.route("/api/tag-counts", methods=["GET"])
def get_metadata_tag_counts():
    return jsonify({"tag_counts": data_loader.get_metadata_tag_counts()})
